...
# --- Bandera para asegurar que la BBDD se inicializa solo una vez ---
_db_initialized = False

def init_db():
    """Crea la tabla 'partidos' si no existe."""
    global _db_initialized
    if _db_initialized:
        return # Ya se ha inicializado, no hacer nada
    ...
    try:
        ...
        _db_initialized = True # ¡Marcar como inicializada!
...
def ensure_db_initialized():
    """Se ejecuta antes de CADA petición, pero init_db() solo se ejecuta una vez."""
    init_db()
    ...
    if isinstance(raw_boxscore, list):
        for j in raw_boxscore:
            ...
            processed_boxscore.append({
                "number": j.get('number', ''), "name": j.get('name', ''),
                "pts": pts, "tc": f"{tc}/{tci}", "pct": pct,
                "3p": f"{TPM}/{TPA}", "3ppct": tpp, "tl": f"{FTM}/{FTA}", "tlpct": ftp,
                "reb": s.get('REB', 0), "ast": s.get('AST', 0), "stl": s.get('STL', 0),
                "blk": s.get('BLK', 0), "tov": s.get('TOV', 0), "pf": s.get('PF', 0)
            })
    
    processed_boxscore.sort(key=lambda x: x.get('number', 0))
    return render_template("detalle_partido.html", partido=partido_data, boxscore=processed_boxscore)
...
def guardar_boxscore():
    try:
        ...
        with engine.connect() as conn:
            conn.execute(query, {
                "fecha": fecha, "rival": rival, "marcador_local": marcador_local, 
                "marcador_rival": marcador_rival, "boxscore_json": boxscore_json
            })
...
if __name__ == "__main__":
    # init_db() se llama ahora a través del hook before_request
